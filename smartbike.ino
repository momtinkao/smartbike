...
char ssid[] = "派大星";   // your network SSID (name) 
...
void onReceive(int packetSize)
{
  ...
  Latitude[0] = LoRa.read();
  Latitude[1] = LoRa.read();
  Latitude[2] = LoRa.read();
  Latitude[3] = LoRa.read();
  Longitude[0] = LoRa.read();
  Longitude[1] = LoRa.read();
  Longitude[2] = LoRa.read();
  Longitude[3] = LoRa.read();
  byte incomingLength = LoRa.read();

  float Lat;
  ((uint8_t*)&Lat)[0] = Latitude[0];
  ((uint8_t*)&Lat)[1] = Latitude[1];
  ((uint8_t*)&Lat)[2] = Latitude[2];
  ((uint8_t*)&Lat)[3] = Latitude[3];
  float Lng;
  ((uint8_t*)&Lng)[0] = Longitude[0];
  ((uint8_t*)&Lng)[1] = Longitude[1];
  ((uint8_t*)&Lng)[2] = Longitude[2];
  ((uint8_t*)&Lng)[3] = Longitude[3];
  char Lats[10];
  char Lngs[10];
  sprintf(Lats,"%.6f",Lat);
  sprintf(Lngs,"%.6f",Lng);
  ...
  String incoming = String(Lat) + String(Lng);
  ...
  Serial.print("lat: ");
  Serial.println(Lats);
  ThingSpeak.setField(1, Lats);
  ThingSpeak.setField(2, Lngs);
